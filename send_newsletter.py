...
def fetch_supabase_subscribers() -> list[str]:
    """Supabase subscribers 테이블에서 구독 이메일을 읽는다 (service_role 키 필요).
    SUPABASE_URL/SUPABASE_SERVICE_KEY 미설정이면 빈 목록을 반환한다."""
    ...
    try:
        req = urllib.request.Request(
            f"{url}/rest/v1/subscribers?select=email",
        ...
        print(f"[경고] Supabase 구독자 조회 실패 — env 목록만 사용: {e}")
        return []
...
def main() -> int:
    ...
    env_recipients = parse_recipients(os.environ.get("NEWSLETTER_RECIPIENTS", ""))
    # env 명단 + Supabase 구독자 병합 (중복 제거, 형식 검증)
    merged: dict[str, None] = {a: None for a in env_recipients}
    sb = fetch_supabase_subscribers()
    for addr in sb:
        if _EMAIL_RE.match(addr):
            merged.setdefault(addr, None)
    recipients = list(merged)
    if sb:
        print(f"구독자: env {len(env_recipients)}명 + Supabase {len(sb)}명 → 총 {len(recipients)}명")
