...
<h2>2. 계정과 로그인</h2>
<p>로그인은 구글 계정(Google OAuth)으로 제공됩니다. 로그인 시 스크랩북과 '뉴스 다이어트'(읽은 기사의 성향 기록)는 이용자 계정에 연결되어 서버(Supabase)에 저장되고 기기 간에 동기화됩니다. 로그인하지 않으면 해당 데이터는 이용 중인 기기(브라우저)에만 저장됩니다. 개인정보 처리의 자세한 내용은 <a class="text-blue-500" href="privacy.html">개인정보처리방침</a>을 따릅니다.</p>
<h2>3. 이용자 콘텐츠(댓글)</h2>
<p>이슈에 남기는 댓글(의견)은 <b>익명으로 공개 저장</b>됩니다. 욕설·혐오·차별·폭력 선동·광고·개인정보 노출 등 부적절한 내용은 사전·사후 검열로 등록이 거부되거나 삭제될 수 있습니다. 댓글을 게시하면 해당 내용을 서비스가 공개·보관하는 데 동의하는 것으로 봅니다.</p>
<h2>4. 금지 행위</h2>
...
<ul>
<li><b>구글 로그인</b> — 구글 계정으로 로그인하면 이메일 주소, 이름(프로필 표시명), 계정 고유 식별자를 제공받아 로그인 상태 유지와 이용자 식별에 사용합니다. 인증은 Google OAuth와 Supabase Auth로 처리되며, 비밀번호는 저장하지 않습니다.</li>
<li><b>스크랩북</b> — 로그인 시 스크랩한 이슈·글이 계정에 연결되어 서버에 저장되고 기기 간 동기화에 사용됩니다.</li>
<li><b>뉴스 다이어트(읽은 기사 성향 기록)</b> — 로그인 시 클릭한 기사의 성향 분류가 계정에 연결되어 서버에 저장됩니다. 이는 열람 이력에 해당하는 민감할 수 있는 정보로, 본인만 접근하도록 행 수준 접근제어(RLS)를 적용하며 개인화 대시보드 제공 목적에만 사용합니다.</li>
<li><b>비로그인 이용</b> — 로그인하지 않으면 스크랩·다이어트·키워드 알림·글자 크기는 이용 기기의 브라우저 저장소(localStorage)에만 저장되며 서버로 전송되지 않습니다.</li>
</ul>
<h2>2. 댓글(의견)</h2>
<p>이슈에 남기는 댓글은 <b>계정과 연결하지 않고 익명으로</b> 공개 저장됩니다(작성자 신원을 기록하지 않습니다). 검열을 위해 작성 내용이 자동/수동으로 검토될 수 있습니다.</p>
...
def build_scrapbook_page(
  ...
  <section id="scrap-posts-sec" hidden>
    ...
    page = _page(
        ...
        after_header="",
        main_html=main_html,
        footer_notes=["스크랩은 로그인 시 계정에 저장되어 기기 간에 동기화되며, 비로그인 시에는 이 브라우저에만 저장됩니다."],
        site_stamp=stamp,
...
def build_diet_page(out_dir: Path, generated_at: str, now: datetime, updated: str, stamp: str) -> Path:
  ...
  <section id="diet-main" hidden class="grid gap-5 lg:grid-cols-2">
    ...
    page = _page(
        ...
        canonical="diet.html",
        description="내가 읽은 뉴스의 성향 분포를 돌아보는 개인 대시보드 (로그인 시 계정에 동기화).",
        active="diet",
        generated_at=generated_at,
        feed="",
        updated_label=updated,
        ...
        after_header="",
        main_html=main_html,
        footer_notes=["읽은 기사 기록은 비로그인 시 이 브라우저에만 저장되며, 로그인 시 계정에 저장되어 기기 간에 동기화됩니다."],
        site_stamp=stamp,
